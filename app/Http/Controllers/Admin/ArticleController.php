...
class ArticleController extends Controller
{
    ...
    {
        return Article_group::with('keywords')->get();
    }
    ...
    {
        $article = Article_group::where('id', $request->groupid)->with('articles.user.userinfo')->with('articles.keywords')->get();
    ...
    {
        $contentid = Article_content::select('id')->orderBy('id', 'desc')->first();

        $randomnum = rand(1000, 9999);
        $image = new ImageManager();
        $image->make($request->image->getRealPath())->save(public_path() . '/media/gallery/' . $contentid['id'] . '/gallery_' . $randomnum . '.png');
        $image->make($request->image->getRealPath())->resize('120', '120')->save(public_path() . '/media/gallery/' . $contentid['id'] . '/gallerysmall_' . $randomnum . '.png');

        $gallery = Gallery::create([
            'article_content_id' => $contentid['id'],
            'image' => $randomnum,
        ]);

        return [$randomnum, $contentid['id'], $gallery['id']];
    }
    ...
    public function deletegalleryimage(Request $request)
    {
        ...
        Storage::disk('media')->delete('gallery/' . $gallery['article_content_id'] . '/gallery_' . $gallery['image'] . '.png');
        Storage::disk('media')->delete('gallery/' . $gallery['article_content_id'] . '/gallerysmall_' . $gallery['image'] . '.png');

        Gallery::where('id', $request->id)->delete();

        $otherchild = Gallery::where('article_content_id', $gallery['article_content_id'])->get();

        if (count($otherchild) == 0) {
            Article_content::where('id', $gallery['article_content_id'])->delete();
            Storage::disk('media')->deleteDirectory('gallery/' . $gallery['article_content_id']);
        }
